    ...
    for(i = 0; i < x.rows; ++i){
        for(j = 0; j < x.cols; ++j){
            norm.data[i*x.cols + j] = 
                (x.data[i*x.cols + j]-m.data[j/n])/(sqrt(v.data[j/n])+0.00001);
        }
    }
    return norm;
}
    ...
    for(i = 0; i < d.rows; ++i){
        for(j = 0; j < d.cols; ++j){
            dv.data[j/n] += 
                ...
                *(-0.5*(pow(v.data[j/n]+0.00001, -1.5)));
        }
    }
    return dv;
}
    ...
    for(i = 0; i < d.rows; ++i){
        for(j = 0; j < d.cols; ++j){
            dx.data[i*d.cols + j] = 
                ...
                +dv.data[j/n]*((2*(x.data[i*d.cols + j]-m.data[j/n]))/(n*d.rows))
                +dm.data[j/n]/(n*d.rows);
        }
    }
    return dx;
}
